...
namespace Antares::Action::AntaresStudy::Area {
    ...
    Create::Create(const AnyString &areaname, const AnyString &targetname) : pOriginalAreaName(areaname),
    ...
    bool Create::performWL(Context &ctx) {
        ...
        if (not ctx.area) {
            // create the area
            UpdateStrategy strategy(*ctx.study.get());
            ctx.area = ctx.study->areaAdd(pFuturAreaName, &strategy);
